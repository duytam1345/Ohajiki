...
function firebase_copy(){
	...
	if [ $? -ne 0 ]; then
		echo Error! at firebase file copy
...
if [ $? -ne 1 ]; then
	echo Error! in ${WORKSPACE}/${BUILD_NUMBER}_fb_XXX.log
	exit 1
fi
...
grep -i login ${WORKSPACE}/${BUILD_NUMBER}.log | grep -i fail | grep -v "level=warning"
if [ $? -ne 1 ]; then
	echo Error! in ${WORKSPACE}/${BUILD_NUMBER}.log
	exit 1
fi
